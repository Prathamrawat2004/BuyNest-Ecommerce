...
const SearchRes = () => {
    ...
    const fetchImages = async (query) => {
        try {
            const response = await fetch(`https://api.unsplash.com/search/photos?query=${query}&client_id=xOHrHJZjKnryd8O-V-8c-bBVxxxamsk9Tzm5VWdwjhM&per_page=16`);
    ...
    return (
        <>
            ...
            <div className={`first-image d-flex card-item ${darkmode ? "text-white" : ""}`}>
                {images.length > 0 && (
                    <img src={images[0].urls.regular} className='rounded' height={220} width={350} alt={images[0].alt_description || 'Image'} />
                )}
                <div className="first-content">
                    {images.length > 0 && (
                        <>
                            <span className='mt-3'>HMMARBLE(1,290)</span>
                            <h3>{images[0].alt_description || 'Default Description'}</h3>
